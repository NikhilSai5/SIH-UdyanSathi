...
def raisePyAutoGUIImageNotFoundException(wrappedFunction):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            # Raise PyAutoGUI's ImageNotFoundException.
...
def click(
    ...
    _logScreenshot(logScreenshot, "click", "%s,%s,%s,%s" %
                   (button, clicks, x, y), folder=".")
...
def leftClick(x=None, y=None, interval=0.0, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    click(x, y, 1, interval, LEFT, duration,
          tween, logScreenshot, _pause=_pause)
...
def rightClick(x=None, y=None, interval=0.0, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    Returns:
      None
    """
    click(x, y, 1, interval, RIGHT, duration,
          tween, logScreenshot, _pause=_pause)
...
def middleClick(x=None, y=None, interval=0.0, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    Returns:
      None
    """
    click(x, y, 1, interval, MIDDLE, duration,
          tween, logScreenshot, _pause=_pause)
...
def doubleClick(x=None, y=None, interval=0.0, button=LEFT, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    if sys.platform == "darwin":
        ...
        _logScreenshot(logScreenshot, 'click', '%s,%s,%s,2' %
                       (x, y, button), folder='.')
    else:
        # Click for Windows or Linux:
        click(x, y, 2, interval, button, duration,
              tween, logScreenshot, _pause=False)
...
def tripleClick(x=None, y=None, interval=0.0, button=LEFT, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    if sys.platform == "darwin":
        ...
        _mouseMoveDrag("move", x, y, 0, 0, duration=0, tween=None)
        x, y = platformModule._position()
        _logScreenshot(logScreenshot, "click", "%s,%s,%s,3" %
                       (x, y, button), folder=".")
        platformModule._multiClick(x, y, button, 3)
    else:
        # Click for Windows or Linux:
        click(x, y, 3, interval, button, duration,
              tween, logScreenshot, _pause=False)
...
def scroll(clicks, x=None, y=None, logScreenshot=None, _pause=True):
    ...
    _logScreenshot(logScreenshot, "scroll", "%s,%s,%s" %
                   (clicks, x, y), folder=".")
...
def hscroll(clicks, x=None, y=None, logScreenshot=None, _pause=True):
    ...
    _logScreenshot(logScreenshot, "hscroll", "%s,%s,%s" %
                   (clicks, x, y), folder=".")
...
def vscroll(clicks, x=None, y=None, logScreenshot=None, _pause=True):
    ...
    _logScreenshot(logScreenshot, "vscroll", "%s,%s,%s" %
                   (clicks, x, y), folder=".")
...
def moveRel(xOffset=None, yOffset=None, duration=0.0, tween=linear, logScreenshot=False, _pause=True):
    ...
    _logScreenshot(logScreenshot, "moveRel", "%s,%s" %
                   (xOffset, yOffset), folder=".")
...
def dragRel(
    ...
    _logScreenshot(logScreenshot, "dragRel", "%s,%s" %
                   (xOffset, yOffset), folder=".")
    if mouseDownUp:
        mouseDown(button=button, logScreenshot=False, _pause=False)
    _mouseMoveDrag("drag", mousex, mousey, xOffset,
                   yOffset, duration, tween, button)
    if mouseDownUp:
        mouseUp(button=button, logScreenshot=False, _pause=False)
...
def _mouseMoveDrag(moveOrDrag, x, y, xOffset, yOffset, duration, tween=linear, button=None):
    ...
    assert moveOrDrag in (
        "move", "drag"), "moveOrDrag must be in ('move', 'drag'), not %s" % (moveOrDrag)
    ...
    if duration > MINIMUM_DURATION:
        ...
        steps = [getPointOnLine(startx, starty, x, y, tween(
            n / num_steps)) for n in range(num_steps)]
...
def hotkey(*args, **kwargs):
    ...
    _logScreenshot(kwargs.get("logScreenshot"),
                   "hotkey", ",".join(args), folder=".")
    for c in args:
        if len(c) > 1:
            c = c.lower()
...
def displayMousePosition(xOffset=0, yOffset=0):
    ...
    try:
        while True:
            ...
            x, y = position()
            positionStr = "X: " + \
                str(x - xOffset).rjust(4) + " Y: " + str(y - yOffset).rjust(4)
...
def _getNumberToken(commandStr):
    ...
    if mo is None:
        raise PyAutoGUIException(
            "Invalid command at index 0: a number was expected")
...
def _getCommaToken(commandStr):
    ...
    if mo is None:
        raise PyAutoGUIException(
            "Invalid command at index 0: a comma was expected")
...
def _tokenizeCommandStr(commandStr):
    ...
    while i < len(commandStr):
        ...
        if mo is None:
            raise PyAutoGUIException(
                "Invalid command at index %s: %s is not a valid command" % (i, commandStr[i]))
        ...
        if individualCommand in ("c", "l", "m", "r", "su", "sd", "ss"):
            # This just exists so these commands are covered by one of these cases.
            pass
        ...
        # Handle the arguments of the mouse (g)o and mouse (d)rag commands:
            ...
            try:
                ...
                # Increment past the comma (and any whitespace).
                i += len(comma)
            ...
            if x.lstrip()[0].isdecimal() and not y.lstrip()[0].isdecimal():
                raise PyAutoGUIException(
                    "Invalid command at index %s: Y has a +/- but X does not." % (i - len(y)))
        ...
        # Handle the arguments of the (f)or loop command:
            ...
            try:
                ...
                # Increment past the sub-command string.
                i += len(subCommandStr)
...
_right, _bottom = size()
FAILSAFE_POINTS.extend(
    [(0, _bottom - 1), (_right - 1, 0), (_right - 1, _bottom - 1)])
