        ...
        if key in mapping:
            if not self.allow_duplicate_keys:
                ...
                if self.allow_duplicate_keys is None:
                    warnings.warn(DuplicateKeyFutureWarning(
                        *args), stacklevel=1)
                else:
    ...
    def construct_yaml_float(self, node: Any) -> float:
        ...
        if value_s == '.inf':
            ...
            if self.resolver.processing_version != (1, 2) and 'e' in value_s:
                ...
                if '.' not in mantissa:
                    warnings.warn(MantissaNoDotYAML1_1Warning(
                        node, value_so), stacklevel=1)
...
class RoundTripConstructor(SafeConstructor):
        ...
        if node.style == '|' and isinstance(node.value, str):
            ...
            if self.loader and self.loader.comment_handling is None:
                ...
                if node.comment is not None and node.comment[1]:
                    ...
                    # EOL comment after |
                    lss.comment = self.comment(
                        node.comment[1][0])  # type: ignore
            return lss
        if node.style == '>' and isinstance(node.value, str):
        ...
        for key_node, value_node in node.value:
            ...
            if not isinstance(key, Hashable):
                if isinstance(key, MutableSequence):
                    ...
                    key_s._yaml_set_line_col(
                        key.lc.line, key.lc.col)  # type: ignore
                    key = key_s
                ...
                if self.loader and self.loader.comment_handling is None:
                    ...
                    if value_node.comment:
                        ...
                        if value_node.comment[0]:
                            maptyp.ca.set(key, C_VALUE_PRE,
                                          value_node.comment[0])
                        if value_node.comment[1]:
                            maptyp.ca.set(key, C_VALUE_EOL,
                                          value_node.comment[1])
                        if value_node.comment[2]:
                            maptyp.ca.set(key, C_VALUE_POST,
                                          value_node.comment[2])
    ...
    def construct_yaml_timestamp(
    ...
    ) -> Union[datetime.datetime, datetime.datetime, TimeStamp]:
        try:
            match = self.timestamp_regexp.match(node.value)
        except TypeError:
            match = None
        ...
        # isinstance(datetime.datetime.now, datetime.datetime) is true)
